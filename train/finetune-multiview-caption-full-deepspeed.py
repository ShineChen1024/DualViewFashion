...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="FLUX Fill full finetune with per-sample captions.")
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional path to initialize the trainable transformer from a previous full finetune. "
            "Pass either a transformer directory or a model directory containing a transformer subdirectory. "
            "Tokenizers, text encoders, scheduler, and VAE are still loaded from pretrained_model_name_or_path."
        ),
    )
    ...
    parser.add_argument("--output_dir", type=str, default="flux-multiview-full")
...
def main(args):
    ...
    local_device = torch.device("cuda", accelerator.local_process_index)
    ...
    vae.to(local_device, dtype=weight_dtype)
    text_encoder_one.to(local_device, dtype=weight_dtype)
    text_encoder_two.to(local_device, dtype=weight_dtype)

    # Build dataset and pre-encode all unique prompts
    ...
    with accelerator.main_process_first():
        transformer_source = args.transformer_model_name_or_path or args.pretrained_model_name_or_path
        transformer_subfolder = "transformer"
        if args.transformer_model_name_or_path:
            transformer_subfolder = (
                "transformer"
                if os.path.isdir(os.path.join(args.transformer_model_name_or_path, "transformer"))
                else None
            )
        ...
        logger.info(
            "Loading trainable transformer from %s%s",
            transformer_source,
            f"/{transformer_subfolder}" if transformer_subfolder else "",
        )
    ...
    all_parameters = list(filter(lambda p: p.requires_grad, transformer.parameters()))
    params_to_optimize = [{"params": all_parameters, "lr": args.learning_rate}]
    ...
    if accelerator.is_main_process:
        tracker_name = "flux-dev-full-multiview-caption"
        accelerator.init_trackers(tracker_name, config=vars(args))

    total_batch_size = args.train_batch_size * accelerator.num_processes * args.gradient_accumulation_steps

    logger.info("***** Running training (full finetune) *****")
    ...
    def get_sigmas(timesteps, n_dim=4, dtype=torch.float32):
        sigmas = noise_scheduler_copy.sigmas.to(device=local_device, dtype=dtype)
        schedule_timesteps = noise_scheduler_copy.timesteps.to(local_device)
        timesteps = timesteps.to(local_device)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                prompt_embeds = cached_prompt_embeds[p_indices].to(local_device, dtype=weight_dtype)
                pooled_prompt_embeds = cached_pooled_prompt_embeds[p_indices].to(local_device, dtype=weight_dtype)
                text_ids = cached_text_ids.to(local_device)
                ...
                if unwrap_model(transformer).config.guidance_embeds:
                    guidance = torch.tensor([args.guidance_scale], device=local_device)
    ...
    if accelerator.is_main_process:
        ...
        logger.info(f"Full model saved to {os.path.join(args.output_dir, 'transformer')}")
