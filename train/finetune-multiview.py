...
def parse_args(input_args=None):
    parser = argparse.ArgumentParser(description="FLUX Fill LoRA training for multi-view model generation.")
    ...
    parser.add_argument("--jsonl_path", type=str,
                        default="/home/work/MMSearch/cwf/search/multi-view/all_two_front_accept_with_candidate_corrected.jsonl")
...
def collate_fn(examples):
    ...
    return {"pixel_values": pixel_values, "masks": masks, "prompts": prompts}
...
def main(args):
    ...
    # === debug: log LoRA config and parameter stats ===
    ...
    logger.info("Pre-encoding fixed prompt and freeing text encoders...")
    with torch.no_grad():
        cached_prompt_embeds, cached_pooled_prompt_embeds, cached_text_ids = encode_prompt(
            text_encoders, tokenizers, PROMPT, args.max_sequence_length
        )
        cached_prompt_embeds = cached_prompt_embeds.to(accelerator.device)
        cached_pooled_prompt_embeds = cached_pooled_prompt_embeds.to(accelerator.device)
        cached_text_ids = cached_text_ids.to(accelerator.device)
    ...
    if accelerator.is_main_process:
        tracker_name = "flux-dev-lora-multiview"
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                bsz_actual = batch["pixel_values"].shape[0]
                prompt_embeds = cached_prompt_embeds.expand(bsz_actual, -1, -1)
                pooled_prompt_embeds = cached_pooled_prompt_embeds.expand(bsz_actual, -1)
                text_ids = cached_text_ids
