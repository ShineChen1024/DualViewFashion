...
def parse_args():
    ...
    parser.add_argument("--prompt", default=None, help="Override the automatically built generation prompt.")
...
def build_prompt(args):
    ...
    prompt = (
        f"a 7-grid fashion generation layout: the top row shows the {args.cloth_type} "
        f"front view, the {args.cloth_type} back view, and a model identity placeholder currently filled with neutral gray; "
        f"the bottom row shows a {args.gender} fashion model wearing the same {args.cloth_type} "
        "in four synchronized views: reference pose, front view, side view, and back view"
    )

    if args.other_clothing and args.other_clothing.lower() != "none":
        prompt += f", styled with {args.other_clothing}"

    reference_notes = []
    if args.ref_framing and args.ref_framing.lower() != "same":
        reference_notes.append(f"{args.ref_framing} framing")
    if args.ref_extra_layers and args.ref_extra_layers.lower() != "none":
        reference_notes.append(f"additional {args.ref_extra_layers}")
    if reference_notes:
        prompt += f"; preserve the garment despite reference details such as {', '.join(reference_notes)}"

    prompt += (
        ". Preserve garment identity, color, print, silhouette, and back-view details across all generated views."
    )
    return prompt
